...
A random fixed encoder — a frozen random orthogonal projection with no semantic content — outperforms the free encoder by 17×, confirming that coordinate stability alone provides a substantial advantage even without task-relevant alignment. A completed 2×2 factorial design reveals that the two factors — stability and alignment — are not independent: an encoder initialized on the exact ground-truth coordinates but allowed to drift performs no better than free (and sometimes worse). Stability is a prerequisite; alignment contributes only when stability is maintained.

These findings establish, through direct experimental controls, that the advantage of prescribed representations involves at least two contributing factors: temporal stability of coordinates and their alignment with task-relevant quantities. In this setting, the collapse problem is not about preserving geometric properties of the space, but about maintaining an identifiable and stable coordinate system in which downstream modules can accumulate learning. Additional experiments confirm that drift is not a tuning artifact (prescribed wins at all four tested learning rates, 4.3–7.0×), that EMA target encoders do not fix the problem (6.1× worse than prescribed), and that the same drift pattern appears in vision (Rico UI) — confirming cross-modal generality.
...
5. We isolate these factors with a completed 2×2 factorial design. A random fixed encoder (no semantic content) outperforms free by 17×. An aligned-but-drifting encoder (initialized on ground-truth coordinates, then allowed to train) performs no better than free. This establishes that stability is a prerequisite for alignment to have any effect, not an independent contributing factor.
...
**Approximate multiplicative decomposition.** The total gap between free and prescribed is 233×. In this configuration, it decomposes approximately into stability (17.4×) × alignment (13.4×) = 233×. However, this factorization relies on three data points, not a full factorial design. Section 5.5 fills the missing cell.
...
### 5.6 Encoder Learning Rate Sweep

A natural objection: perhaps drift is a tuning artifact — the free encoder simply uses the wrong learning rate. We test four LR values {1e-4, 3e-4, 1e-3, 3e-3} on Push-T with SIGReg, seed=42, 50 epochs.

| Condition | Best Val Loss | Ratio vs prescribed | R²(0→1) |
|---|---|---|---|
| Prescribed (LR=3e-4) | 0.099 | 1.0× | 1.0000 |
| Free LR=1e-4 | 0.689 | 7.0× | 0.8459 |
| Free LR=3e-4 | 0.510 | 5.2× | 0.7801 |
| Free LR=1e-3 | 0.467 | 4.7× | 0.7600 |
| Free LR=3e-3 | 0.420 | 4.3× | 0.7437 |

Prescribed wins at every LR. The gap ranges from 4.3× to 7.0× — not a tuning problem. R² transfer on epoch 0→1 is below 0.85 for all free conditions, confirming that drift occurs regardless of learning rate. Higher LR produces better final loss (the encoder escapes bad regions faster) but does not eliminate the structural disadvantage.

### 5.7 EMA Target Encoder Baseline

EMA target encoders (as in I-JEPA, BYOL) provide slowly-moving targets to reduce representation drift. We test free encoder + EMA (decay=0.996) on the same Push-T setup.

| Condition | Best Val Loss | Ratio vs prescribed |
|---|---|---|
| Prescribed | 0.099 | 1.0× |
| Free + EMA | 0.604 | 6.1× |
| Free (LR=3e-4) | 0.510 | 5.2× |

EMA does not help — it is 6.1× worse than prescribed and worse than plain free (5.2×). EMA stabilizes the *target* representation but does not fix the *online encoder's* coordinate system. The additional inertia slows adaptation without preventing drift. Prescribed axes are the limiting case of EMA with decay=1.0 and perfect initialization — but the benefit comes from fixing the right coordinates, not from slowing change.
...
We now test both an EMA target encoder baseline (Section 5.7) and an encoder learning rate sweep (Section 5.6). Neither eliminates the gap: EMA performs 6.1× worse than prescribed (and worse than plain free), while prescribed wins at all four tested LR values (4.3–7.0×). Cross-modal validation on Rico UI (Section 5.8) confirms drift in vision, though weaker than in Push-T.

The completed 2×2 factorial design (Section 5.5) shows that stability and alignment interact strongly — they are not independent factors. The stability effect is 330× among aligned encoders but only 17.5× among unaligned ones. This interaction means the original multiplicative decomposition (17× × 13× = 233×) was an approximation that obscured the hierarchical relationship between the factors. The correct structure — stability as prerequisite, alignment as additional benefit — is established in this setting but may differ in higher-dimensional spaces where drift dynamics could be qualitatively different.
...
Prescribed axes solve both problems simultaneously: they provide coordinates that are stable (they never change) and aligned (they correspond to task-relevant quantities). The completed factorial design reveals the relationship between these factors: stability is a prerequisite, not merely a contributing factor. An encoder initialized on perfect coordinates but allowed to drift loses its entire advantage — performing no better than a randomly initialized free encoder. Conversely, a random but stable projection outperforms the free encoder by 17×. Alignment contributes an additional 12× — but only when stability is maintained. Rotated prescribed axes perform identically to prescribed (1.09×), confirming that interpretability of axes is irrelevant — what matters is fixedness and subspace selection.

Whether this pattern generalizes beyond low-dimensional world models is an open question. But the result establishes a concrete, experimentally grounded alternative to the rank-collapse narrative: the goal of representation learning may be not to fill the space uniformly or use all dimensions, but to establish a coordinate system that is identifiable and stable enough for other modules to build on — and aligned enough with the task to make building worthwhile. An encoder learning rate sweep (4 values, 4.3–7.0× gap), an EMA baseline (6.1× gap), and cross-modal validation on Rico UI vision confirm that this is a structural phenomenon, not a tuning artifact or domain-specific effect.

---

## 8. Reproducibility

Three seeds (42, 123, 777), 200 episodes per seed collected via gym-pusht with pymunk physics, 30 epochs, SIGReg λ=0.09. LR sweep: seed 42, 50 epochs, LR={1e-4, 3e-4, 1e-3, 3e-3} + EMA (decay=0.996). Rico UI drift: seed 42, 100 epochs, Rico dataset (500 samples), free 3D+SIGReg vs ShovJEPA. All analysis code, notebooks, data collection scripts, and results are available at https://github.com/revenue7-eng/prescribed-axes-drift.
